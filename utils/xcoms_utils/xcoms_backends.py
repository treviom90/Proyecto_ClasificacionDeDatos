...
class PickleXComBackend(XComObjectStorageBackend):
    """
    Backend personalizado que guarda en disco (XCOM_OBJECTSTORAGE_PATH)
    los objetos serializados con pickle. Devuelve los bytes originales
    al hacer pull(), para que luego puedas aplicar pickle.loads().
    """
    ...
    def serialize_value(value, key=None, **kwargs):
        ...
        filename = f"{key or 'xcom'}_{run_id}_{ts}.bin"
    ...
    def deserialize_value(result):
        ...
        # 2) Devolver los bytes crudos (para hacer pickle.loads() después)
        return payload
